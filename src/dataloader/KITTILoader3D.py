...
def dataloader(filepath, train_file, val_file, kitti2015=False):
    ...
    if kitti2015:
        ...
        calib_train = [filepath + '/' + calib + img + '.txt' for img in train_idx]
        mask_train = [filepath + '/' + mask_fold + img + '.png' for img in train_idx]
        label_train = [filepath + '/' + label_fold + img + '.txt' for img in train_idx]
        ...
        calib_val = [filepath + '/' + calib + img + '.txt' for img in val_idx]
        mask_val = [filepath + '/' + mask_fold + img + '.png' for img in val_idx]
        label_val = [filepath + '/' + mask_fold + img + '.txt' for img in val_idx]



    return [left_train, right_train, depth_train, calib_train,mask_train, label_train], \
           [left_val, right_val, depth_val, calib_val,mask_val,label_val]
