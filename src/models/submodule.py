...
class depthregression(nn.Module):
    def __init__(self, maxdepth, depth_min):
        ...
        self.disp = torch.linspace(depth_min[0], depth_min[1], 40, device='cuda',requires_grad=False).float()[None, :, None, None]
...
class feature_extraction(nn.Module):
    def __init__(self):
        ...
        self.branch1 = nn.Sequential(nn.AvgPool2d((56, 56), stride=(56, 56)),
                                     convbn(128, 32, 1, 1, 0, 1),
                                     nn.ReLU(inplace=True))
        self.branch2 = nn.Sequential(nn.AvgPool2d((28, 28), stride=(28, 28)),
                                     convbn(128, 32, 1, 1, 0, 1),
                                     nn.ReLU(inplace=True))
        self.branch3 = nn.Sequential(nn.AvgPool2d((14, 14), stride=(14, 14)),
                                     convbn(128, 32, 1, 1, 0, 1),
                                     nn.ReLU(inplace=True))

        self.branch4 = nn.Sequential(nn.AvgPool2d((7, 7), stride=(7, 7)),
                                     convbn(128, 32, 1, 1, 0, 1),
                                     nn.ReLU(inplace=True))
