...
class Tester(object):
    ...
    def test_one_epoch(self):
        ...
        for _, data in enumerate(self.dataloader):
            ...
            conf= torch.sigmoid(outputs['conf']).cpu().numpy()
            ...
            for i in range(batch_size):
                ...
                heading_prob = np.max(softmax(outputs['heading_scores'][i, :]))
                size_prob = np.max(softmax(outputs['size_scores'][i, :]))
                ...
                score_list.append(rgb_prob[i]*conf[i])  # 2D RGB detection score  *conf[i]
                id_list.append(id[i])
