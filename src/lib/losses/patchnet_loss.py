...
from lib.utils.fpointnet_utils import box3d_iou
...
def get_loss(center_label,
    ...
    iou3d_gt_list = []
    for i in range(corners_3d_pred.shape[0]):
        iou3d_gt,_ = box3d_iou(corners_3d_pred[i].detach().cpu().numpy(), corners_3d_gt[i].detach().cpu().numpy())
        iou3d_gt_list.append(iou3d_gt)
    iou3d_gt_tensor = torch.from_numpy(np.array(iou3d_gt_list,dtype=np.float32)).cuda()
    # conf_loss = F.binary_cross_entropy(torch.sigmoid(output_dict['conf']), iou3d_gt_tensor, reduction='mean')
    # iou3d_gt=box3d_iou(corners_3d_pred[0].detach().cpu().numpy(), corners_3d_gt[0].detach().cpu().numpy())
