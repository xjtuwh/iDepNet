...
class PatchNet(nn.Module):
    def __init__(self, cfg, num_heading_bin, num_size_cluster, mean_size_arr):
        ...
        self.box_conf = nn.Sequential(nn.Linear(515, 512), nn.BatchNorm1d(512), nn.ReLU(inplace=True),
                                           nn.Dropout(0.5),
                                           nn.Linear(512, 256), nn.BatchNorm1d(256), nn.ReLU(inplace=True),
                                           nn.Dropout(0.5),
                                           nn.Linear(256, 1))
        self.mask1 = nn.Sequential(nn.Conv2d(in_channels=3, out_channels=64,kernel_size=(3,3), padding=(1,1)),
                                   nn.BatchNorm2d(64),
                                   nn.ReLU(inplace=True),
                                   nn.Conv2d(in_channels=64, out_channels=1, kernel_size=(3, 3), padding=(1, 1))
                                   )
        # self.mask2 = nn.Conv2d(in_channels=512, out_channels=1,kernel_size=(1,1), padding=(0,0))
    ...
    def forward(self, patch, one_hot_vec):
        ...
        mask = self.mask1(patch)
        mask = F.softmax(mask.view(mask.shape[0], 1, -1), dim=2)
        att_scores = mask.repeat(1, patch.shape[1], 1)
        f_agg = patch.view(patch.shape[0],patch.shape[1],-1) * att_scores
        mask_xyz_mean = torch.sum(f_agg, dim=2, keepdim=True).unsqueeze(3)

        patch = patch - mask_xyz_mean                                #change the ordinate
        mask_xyz_mean = mask_xyz_mean.squeeze(-1).squeeze(-1)

        # center regressor
        box_features = self.center_reg_backbone(patch)
        # mask1=self.mask1(box_features)
        # att_scores = F.softmax(mask1.view(mask1.shape[0],1,-1), dim=2)
        att_scores1 = mask.repeat(1, box_features.shape[1],1)
        f_agg1 = box_features.view(box_features.shape[0],box_features.shape[1],-1) * att_scores1
        center_reg_features = torch.sum(f_agg1, dim=2)

        # center_reg_features = mask_global_max_pooling_2d(box_features, mask)
        ...
        # mask2 = self.mask2(box_est_features)
        # att_scores1 = F.softmax(mask2.view(mask2.shape[0], 1, -1), dim=2)
        att_scores2 = mask.repeat(1, box_est_features.shape[1], 1)
        f_agg2 = box_est_features.view(box_est_features.shape[0], box_est_features.shape[1], -1) * att_scores2
        box_est_features = torch.sum(f_agg2, dim=2)
        ...
        box_conf = self.box_conf(box_est_features)
        output_dict['conf'] = box_conf
